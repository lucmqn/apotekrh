 ...
 * @author Admin
 */
public class frmCus extends javax.swing.JFrame {
...
// Dalam kelas frmCus
    ...
    // Metode lainnya di frmCus tetap seperti sebelumnya
    ...
    // Ambil ID Customer yang ingin diubah
    ...
    if (customerDAO.updateCustomer(id, customer)) {
        ...
        JOptionPane.showMessageDialog(this, "Gagal mengubah data Customer.");
    }
}
    private void simpan(){
            String action = jLabel1.getText(); // Mendapatkan teks dari JLabel
         if (action.equals("[Tambah]")) {
        saveCustomer(); // Jalankan metode simpan jika teks adalah "Tambah"
        } else if (action.equals("[Ubah]")) {
             updateCustomer(); // Jalankan metode ubah jika teks adalah "Ubah"
       } else {
    ...
    try {
        ...
        String lastCode = customerDAO.getLastKode(duaHurufNama);
